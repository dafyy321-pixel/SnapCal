...
test("README 与应用内说明匹配当前健康和 AI 行为", () => {
  ...
  for (const file of ["CLAUDE.md", "WARP.md"]) {
    const notes = readFileSync(file, "utf8")
    assert.match(notes, /OPENAI_API_KEY/)
    assert.match(notes, /AI_NOT_CONFIGURED/)
    assert.doesNotMatch(notes, /With no key, local mock|Without it, the route returns a deterministic/)
  }
})
