...
test("action card engine", async t => {
  ...
  try {
    ...
    await t.test("source record changes invalidate advice for both day reads and generation", async () => {
      ...
      const mealResponse = await meals.POST(new NextRequest("http://localhost/api/meals", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ meal_name: "午餐", meal_type: "lunch", meal_date: date, meal_time: "12:00:00", calories: 500, protein: 30, carbs: 50, fats: 10 }) }))
      assert.equal(mealResponse.status, 201)
      const { data: { meal } } = await mealResponse.json()
