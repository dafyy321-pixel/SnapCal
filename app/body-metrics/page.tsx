...
function BodyMetricsContent() {
  ...
  return <div className="min-h-screen bg-muted/30 pb-8"><MobilePageHeader title="身体指标" description="分别查看原始趋势，不合成健康分" /><main className="mx-auto max-w-md space-y-4 p-4">
    <Card className="gap-4 p-4"><label className="space-y-1 text-sm font-medium">日期<Input type="date" disabled={busy} value={date} onChange={event => setDate(event.target.value)} /></label><div className="grid grid-cols-2 gap-3">{field("weight_kg", "体重 kg", 20, 500)}{field("waist_cm", "腰围 cm", 30, 300)}{field("body_fat_percent", "体脂率 %", 1, 75)}</div><label className="space-y-1 text-sm font-medium">备注<Textarea maxLength={500} value={form.notes} onChange={event => setForm({ ...form, notes: event.target.value })} /></label><div className="grid grid-cols-2 gap-2">{exists && <Button variant="destructive" disabled={!ready} onClick={remove}><Trash2 />删除</Button>}<Button className={exists ? "" : "col-span-2"} disabled={!ready} onClick={save}><Save />保存指标</Button></div>{loadedDate !== date && <Button variant="outline" onClick={() => setReload(value => value + 1)}>重新加载</Button>}{message && <p role="status" className="text-sm text-muted-foreground">{message}</p>}</Card>
