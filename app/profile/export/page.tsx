...
const formatLabels: Record<Format, string> = { json: "JSON（数据导出）", csv: "CSV（分类表格）" }
...
export default function ExportPage() {
  ...
  return <div className="min-h-screen bg-background pb-24">
    <main className="mx-auto max-w-md space-y-5 px-4 py-6">
      ...
      <Card className="p-5"><div className="flex items-start gap-3"><Database className="mt-0.5 size-6 text-primary" /><div><h2 className="font-semibold">JSON 数据导出</h2><p className="mt-1 text-sm text-muted-foreground">包含资料、餐食、训练动作与组次、状态打卡、身体指标、行动卡和周度尝试。图片文件、自定义训练模板和食物助手会话不在导出范围内；当前不支持 JSON 导入恢复。</p></div></div></Card>
      ...
      <p className="text-xs text-muted-foreground">CSV 中以 =、+、-、@ 开头的内容会自动转义，避免被表格软件当作公式执行。完整备份与恢复请在停止应用后复制整个 data 目录（含数据库和 uploads 图片目录）。</p>
    </main>
    <BottomNav />
  </div>
}
