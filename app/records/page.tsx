...
export default function RecordsPage() {
  ...
  return <div className="min-h-screen bg-muted/30 pb-28"><header className="border-b bg-background px-4 pb-4 pt-10"><div className="mx-auto max-w-md"><h1 className="text-2xl font-bold">记录</h1><p className="text-sm text-muted-foreground">最近 30 天的饮食、训练与身体状态</p></div></header><main className="mx-auto max-w-md space-y-4 p-4"><div className="flex gap-2 overflow-x-auto pb-1">{filters.map(([key, label]) => <button key={key} aria-pressed={filter === key} onClick={() => setFilter(key)} className={`shrink-0 rounded-full border px-4 py-2 text-sm ${filter === key ? "bg-primary text-primary-foreground" : "bg-card"}`}>{label}</button>)}</div>{items.length === 0 && !error && <Card className="p-6 text-center text-sm text-muted-foreground">暂无记录，点击底部添加按钮开始记录</Card>}{items.map((item, index) => { const Icon = icons[item.type]; const showDate = item.date !== items[index - 1]?.date; return <div key={`${item.type}-${item.id}`}>{showDate && <h2 className="mb-2 mt-4 text-sm font-semibold text-muted-foreground">{item.date}</h2>}<button className="w-full text-left" onClick={() => router.push(item.href)}><Card className="mb-2 flex-row items-center gap-3 p-4"><span className="grid size-10 place-items-center rounded-full bg-primary/10"><Icon className="size-5" /></span><span className="min-w-0 flex-1"><span className="block truncate font-medium">{item.title}</span><span className="block truncate text-xs text-muted-foreground">{item.detail}</span></span><span className="text-xs text-muted-foreground">{item.time.slice(0, 5)}</span></Card></button></div>})}{error && <p role="alert" className="text-sm text-destructive">{error}</p>}</main><BottomNav /></div>
}
